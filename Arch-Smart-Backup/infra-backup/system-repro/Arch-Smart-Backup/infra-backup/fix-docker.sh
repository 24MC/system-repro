#!/usr/bin/env bash
# Rezolvare finală pentru eroarea Docker backup
# Problema: scriptul încearcă să copieze fișiere în timpul căutării

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DOCKER_INV="${SCRIPT_DIR}/inventory/docker/inventory.sh"

echo "[INFO] Fixing Docker backup error..."

# Backup
cp "${DOCKER_INV}" "${DOCKER_INV}.backup-$(date +%Y%m%d_%H%M%S)"

# Fix: Adaugă verificare pentru erori în copy_compose_files
cat > "${DOCKER_INV}" << 'DOCKEREOF'
#!/usr/bin/env bash
set -euo pipefail
...
echo "[SUCCESS] Docker inventory fixed!"
echo "[INFO] Now run: ./execution/backup.sh --docker"
